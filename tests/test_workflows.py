...
from jlink_mcp.bridge_models import ProtocolBridgeStatus
...
from test_service import BOARD, PROBE, selector
...
async def test_build_firmware_success_and_failure_artifacts(workflow, monkeypatch) -> None:
    ...
    manifest = Workflows._build_manifest(build)
    assert manifest["embedded_manifest"]["verification"]["ok"]
    assert Workflows._build_artifact(build, "bin").size == 3
    with pytest.raises(RuntimeError, match="no map"):
        Workflows._build_artifact(build, "map")
...
@pytest.mark.asyncio
async def test_protocol_bridge_release_is_reproducible_and_fully_audited(
    workflow, monkeypatch
) -> None:
    source = workflow.settings.workspace_root / "firmware" / "protocol_bridge"
    source.mkdir(parents=True)
    (source / "protocol_bridge.ino").write_text("void setup(){} void loop(){}\n")
    ...
    async def compile_bridge(argv, **kwargs):
        ...
        (output / "protocol_bridge.ino.map").write_text("MAP")
    ...
    assert (generated / "protocol_bridge_m7.hex").read_bytes() == b":deterministic-hex\n"
    assert all("with_bootloader" not in artifact.path for artifact in first.artifacts)
    checked_in = source / "release"
    checked_in.mkdir()
    (checked_in / "protocol_bridge_m7.hex").write_bytes(
        (generated / "protocol_bridge_m7.hex").read_bytes()
    )
    ...
    manifest = json.loads(
        (Path(second.build_directory) / "release" / "protocol_bridge_manifest.json").read_text()
    )
...
@pytest.mark.asyncio
async def test_protocol_bridge_deploy_requires_backup_before_flash(
    workflow, monkeypatch
) -> None:
    source = workflow.settings.workspace_root / "firmware" / "protocol_bridge"
    ...
    source_sha = Workflows._protocol_bridge_source_sha256(source)
    ...
    (release / "protocol_bridge_manifest.json").write_text(json.dumps(manifest))
    (release / "SHA256SUMS").write_text(f"{hex_sha}  protocol_bridge_m7.hex\n")
    ...
    async def backup_flash(address, length, **kwargs):
        events.append(f"backup:{address:#x}:{length:#x}")
    ...
    monkeypatch.setattr(workflow, "flash_and_verify", flash)
    monkeypatch.setattr(workflow.service, "protocol_bridge_status", status)
    ...
    events.clear()
    monkeypatch.setattr(workflow, "backup_flash", failed_backup)
...
async def test_preflight_deploy_and_boot_observation(workflow, manifest, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "jlink_mcp.workflows.inspect_elf",
        lambda path: {
            "test_symbols": {
