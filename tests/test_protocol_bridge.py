...
from jlink_mcp.backends.protocol_bridge import ProtocolBridgeBackend
from jlink_mcp.backends.serial import SerialBackend
from jlink_mcp.bridge_models import (
    BlePairRequest,
    CanSendRequest,
...
from jlink_mcp.bridge_profiles import load_bridge_profiles
from jlink_mcp.bridge_resources import BridgeResourceConflict, BridgeResourceManager
from jlink_mcp.bridge_wire import (
    BridgeCrcError,
    BridgeSequenceError,
    BridgeWireError,
    FieldId,
...
async def test_binary_serial_and_bridge_backend_never_log_payloads(monkeypatch) -> None:
    ...
    monkeypatch.setattr("jlink_mcp.backends.protocol_bridge.secrets.randbits", lambda _: request_id)
...
def test_queue_overflow_and_result_serialization() -> None:
    ...
    result = JLinkService._bridge_result(
        "uart_receive",
        make_result(
