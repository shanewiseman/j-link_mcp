...
class Workflows:
    ...
    async def build_firmware(
        ...
        *,
        ...
        sketch_path = self.settings.resolve_workspace_path(sketch)
        split = flash_split or self.settings.flash_split
    ...
    def _build_manifest(build: BuildResult) -> dict[str, Any]:
        artifact = Workflows._build_artifact(build, "manifest")
    ...
    async def capture_rtt(
        ...
        *,
        ...
        destination = (
            self.settings.state_root
            / "artifacts"
            / f"rtt-{resolved.core.value}-{uuid.uuid4()}.log"
        )
    ...
    async def validate_fixture(
        ...
        *,
        ...
        if not self.settings.test_target_disposable:
